...
DESCMSG = """
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
    | !!! PYTHON PACKAGE FOR CIRCULAR MANHATTAN PLOTTING !!! |
    |                    Kevin Esoh, 2026                    |
    |                   kesohku1@jhmi.edu                    |
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
"""
...
def get_arguments(descmsg: str = DESCMSG) -> argparse.Namespace:
    ...
    opt.add_argument("-cl_size", "--chrom_label_size",  default=6, type=float, metavar="float",
                     help="Chromosome label font size (default: 6).")
    opt.add_argument("-cl_side", "--chrom_label_side", choices=["inside", "outside"],
                     nargs="?", default="inside", const="inside", type=str,
                     help="Chromosome label placement (default: inside).")
    opt.add_argument("-tl_size", "--track_label_size", default=6, type=float, metavar="float",
                     help="Track label font size (default: 6).")
    opt.add_argument("-tl_orient", "--track_label_orientation",
    ...
    opt.add_argument("-st", "--sort_track",
    ...
    opt.add_argument("-hlt", "--highlight_thresh", default=5e-8, type=float, metavar="float",
                     help="P-value threshold for highlighting (default: 5e-8).")
    opt.add_argument("-hll", "--highlight_line", action="store_true",
                     help="Draw vertical lines through highlighted positions.")

    opt.add_argument("--colors", default="steelblue,silver", type=str, metavar="str",
                     help="Two comma-separated alternating chromosome colours.")
    opt.add_argument("--r_min", default=20, type=int, metavar="int",
                     help="Inner radius proportion (circular mode, default: 20).")
    opt.add_argument("--r_max", default=100, type=int, metavar="int",
                     help="Outer radius (circular mode, default: 100).")
    opt.add_argument("--pad",   default=1, type=int, metavar="int",
                     help="Space between tracks (default: 1).")

    opt.add_argument("-plt", "--plot_title", default="MyCMplot", type=str, metavar="str",
                     help="Plot title / output file stem.")
    opt.add_argument("-pts", "--plot_title_size", default=8, type=float, metavar="float",
                     help="Plot title font size (default: 8).")

    opt.add_argument("-od", "--output_dir", default=".", type=Path, metavar="path",
                     help="Output directory (default: current directory).")
    opt.add_argument("-of", "--output_format",
                     choices=["png", "pdf", "svg", "jpg"],
                     default="png", type=str, metavar="str",
                     help="Output image format (default: png).")
    opt.add_argument("--dpi", default=300, type=int, metavar="int",
                     help="Output resolution in DPI (default: 300).")

    opt.add_argument("--force", action="store_true",
                     help="Overwrite existing output files.")

    opt.add_argument("--track_heights", type=str, metavar="str",
